...
![img](/public/imgs/one-big-machine/rust-s3-networking-0.png){: style="width: 80%" }
...
* `i` families - they come with big numbers of ephemeral NVMe drives ("storage optimized") than `d`-suffixed instances. They are very helpful if you have a 50TB database shard to operate on. `i4i`/`i3en` used to be the Intel choices, and `i4g`/`im4gn`/`is4gn` used to be the Graviton2 choices. And then things moved stragetically, jumping generations. It's a little inconsistent to explain, so here I link a [list](https://instances.vantage.sh/?id=47bd2800b86fc0e12489d028e9ea7816008fa241) down here.
  ...
  ![img](/public/imgs/ec2-storage-optimized.png){: style="width: 95%" }
...
* `g4dn`/`g5`/`g6` - they are the family of NVIDIA inference-oriented cards. `g4dn` has NVIDIA T4, `g5` has A10g, and `g6` has L4. Interestingly, `g6` (L4) is less powerful than `g5` (A10g), and priced lower.

* `p3` (going away)/`p4d`/`p5`/`p6` - they are the family of NVIDIA training-oriented cards. `p3` had NVIDIA V100, `p4d` has A100, `p5` has H100, and `p6` has B200. If you are not a large company and have signed a big deal with them, or 3y savings plans, don't use them. Go somewhere else. This is a hot topic, and deserves its own post. I gotta say for a lot of non-LLM workloads, there are A100s around in various market segments, and the new [RTX Pro 6000 Blackwell](https://www.nvidia.com/en-us/products/workstations/professional-desktop-gpus/rtx-pro-6000/) cards are awesome. This is a fast-moving and fast-expiring situation, so we can look back in a year.

## EBS News

There are other families, like `x`/`u` for extreme CPU/RAM ratios, `inf*` and `trn*` for AWS's own Inferentia and Trainium chips, `hpc*` for HPC parallel scientific workloads, `mac*` for MacOS CI/CD purposes, and more. I have not directly used them yet. Although, the `dl1.24xlarge` (Intel's Habana Gaudi training chip) on AWS Marketpace paved my way to directly contact the Habana team to use their Gaudi2 chips for a few months, and got good training results.
...
![img](/public/imgs/io2-gp3.png){: style="width: 55%" }
